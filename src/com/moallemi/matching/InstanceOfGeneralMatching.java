...
/**
 * Samples an instance of the matching problem given the 'model', time periods and 'time periods'
 * as well as the initial size of the population. The data associated with the arrivals and
 * departures is stored in 'allExistingTypes', 'arrivalTimes' and 'departureTimes'. If a particular
 * type remains unmatched, the departure time for it is set at -1.
 *
 * @author Nikhil Bhat
 * @version $Revision: 0.1 $, $Date: 2012-09-20 $
 */

public class InstanceOfGeneralMatching {
   ...
   */
  public InstanceOfGeneralMatching(GeneralMatchingModel model,
                                   int timePeriods,
                                   double ipGeometricParameter,
                                   long seed) {
  ...
  /**
   * Match the items, where two items, one supply and one demand, can be matched if they co-exist
   * in some time-period.
   */
  public double match() {
    if (!isSampled) {
